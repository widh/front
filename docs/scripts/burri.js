...
window.onload = function () {
  if (window.prevOnload) {
    window.prevOnload();
    delete window.prevOnload;
  }
  ...
  const lightSwitch = document.querySelector('[data-id="switch"]');
  ...
  // Changing banner
  let textIter = 0;
  const textBanners = document.querySelector('[data-id="banner"]').querySelectorAll('span');
  const textChanger = () => {
    const nowText = textBanners[(textIter++) % textBanners.length];
    const nextText = textBanners[textIter % textBanners.length];
    nowText.dataset.transparent = 'true';
    window.setTimeout(() => {
      nowText.dataset.removed = 'true';
      nextText.dataset.removed = 'false';
      window.setTimeout(() => {
        nextText.dataset.transparent = 'false';
      }, 20);
    }, 240);
  };
